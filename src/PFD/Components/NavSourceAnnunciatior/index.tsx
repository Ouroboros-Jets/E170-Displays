...
export default class NavSourceAnnunciator extends DisplayComponent<T_NavSourceAnnunciatorProps> {
  private readonly primaryNavSourceRef =
    FSComponent.createRef<SVGTextElement>();
  private readonly primaryNavIdRef = FSComponent.createRef<SVGTextElement>();
  private readonly primaryNavDistanceReadOutRef =
    FSComponent.createRef<SVGTextElement>();
  private readonly primaryNavDistanceTimeEnroute =
    FSComponent.createRef<SVGTextElement>();
  ...
  private readonly checkDistance = (): void => {
    const minutes = Math.round(
      (this.nextTargetDistanceNm / this.trueAirspeed) * 60
    ).toString();
    if (this.activeAnnunciator === Annunciators.FMS) {
      this.primaryNavDistanceTimeEnroute.instance.textContent =
        minutes === "Infinity" ? "∞" : minutes;
    } else {
      this.primaryNavDistanceTimeEnroute.instance.textContent =
        minutes === "Infinity" ? "∞" : minutes;
    }
    ...
    sub
      ...
      .handle((wpMode) => {
        if (wpMode) {
          ...
          this.primaryNavSourceRef.instance.setAttribute(
            "fill",
            `${Colors.PINK}`
          );
          this.primaryNavIdRef.instance.setAttribute("fill", `${Colors.PINK}`);
          this.primaryNavDistanceReadOutRef.instance.setAttribute(
            "fill",
            `${Colors.PINK}`
          );
          this.primaryNavDistanceTimeEnroute.instance.setAttribute(
            "fill",
            `${Colors.PINK}`
          );
        } else {
    ...
    sub
      ...
      .handle((wpDist) => {
        if (this.activeAnnunciator === Annunciators.FMS) {
          ...
          this.primaryNavDistanceReadOutRef.instance.textContent =
            this.nextTargetDistanceNm.toString();
        }
      });
