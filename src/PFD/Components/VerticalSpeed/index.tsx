...
const renderMarkers = (): JSX.Element[] => {
  ...
  markers.push(
    <path
      d={`M 570 ${xAxis + firstBigMarkerOffset} L ${leftBound}  ${
        xAxis + 2 * 2 + firstBigMarkerOffset
      }`}
      stroke={stroke}
      stroke-width={2}
      stroke-linecap="round"
  ...
  for (let y = bigSpacing; y < bigCount * bigSpacing; y += bigSpacing) {
    markers.push(
      <path
        d={`M 570 ${xAxis + y + bigOffset} L ${leftBound}  ${
          xAxis + y + bigOffset + tiltFactor * y * 2 + 4
        }`}
        stroke={stroke}
        stroke-width={2}
        stroke-linecap="round"
  ...
  public onAfterRender(node: VNode): void {
    ...
    sub
      ...
      .handle((vs) => {
        ...
        this.vSpdNeedleRef.instance.setAttribute(
          "d",
          `M 560 ${vSpd + 254} L 630  254`
        );
      ...
      <g>
        ...
        <g ref={this.vSpdBoxRef}>
          ...
          <text
            ...
            font-size={17}
            fill={Colors.GREEN}
            ref={this.vSpdValueRef}
          />
          <path
            d={`M 546 ${xAxis - 9} L 594 ${xAxis - 9} M 546 ${
              xAxis + 9
            } L 594 ${xAxis + 9}`}
            stroke="white"
            stroke-width={2}
