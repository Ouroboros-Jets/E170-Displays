...
export class StallSpeedTape extends DisplayComponent<StallSpeedTapeProps> {
  ...
  private readonly checkStall = (): void => {
    if (!this.onGround) {
      ...
      const stallPosition =
        (this.props.maxSpeed - this.stall) * this.props.stretch +
        this.props.minSpeed * this.props.stretch;

      this.redLsaRef.instance.setAttribute(
        "height",
        `${stallPosition - this.props.minSpeed}`
      );
      this.redLsaRef.instance.style.y = `${
        stallPosition - this.props.minSpeed
      }`;

      this.yellowLsaRef.instance.setAttribute(
        "height",
        `${stallPosition - this.props.minSpeed * 2}`
      );
      this.yellowLsaRef.instance.style.y = `${
        stallPosition - this.props.minSpeed * 2
      }`;
    } else {
      this.redLsaRef.instance.style.visibility = "hidden";
