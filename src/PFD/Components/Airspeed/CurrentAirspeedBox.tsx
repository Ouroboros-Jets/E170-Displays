...
class CurrentAirspeedBox extends DisplayComponent<SelectedAirspeedBoxProps> {
  ...
  private readonly hundredthDigitScrollRef =
    FSComponent.createRef<SVGGElement>();
  ...
  private readonly renderDigitTape = (
    removeZeros?: boolean
  ): SVGTextElement[] => {
  ...
  private readonly vStallCheck = (): void => {
    ...
    if (
      !this.onGround &&
      (this.ias <= this.vstall || this.overspeed <= this.ias)
    ) {
      ...
      !this.onGround &&
      (this.ias <= this.vstall + 10 || this.trend >= this.ias)
    ) {
      for (const ref of this.digitRefs) {
    ...
    sub
      ...
      .handle((ias) => {
        ...
        this.tenthDigitScrollRef.instance.setAttribute(
          "transform",
          `translate(${-1 * verticalScrollsSpacing}, ${Math.max(
            ((ias / 10) % 10) * digitSpacing,
            0
          )})`
        );
        ...
        this.hundredthDigitScrollRef.instance.setAttribute(
          "transform",
          `translate(${-2 * verticalScrollsSpacing}, ${Math.max(
            (Math.floor(ias / 100) % 10) * digitSpacing,
            0
          )})`
        );
