    ...
    return (
      <g>
        <PathWithBlackBackground
          d={`M ${center} ${correctedY - offset - 8} L ${center - 45} ${
            correctedY - offset - 80
          } L ${center - 30} ${correctedY - offset - 80} L ${center} ${
            correctedY - offset - 35
          } L ${center + 30} ${correctedY - offset - 80} L ${center + 45} ${
            correctedY - offset - 80
    ...
    return (
      <g>
        <PathWithBlackBackground
          ...
          } L ${center - 30} ${correctedY - offset + 60} L ${center} ${
            correctedY - offset + 25
          } L ${center + 30} ${correctedY - offset + 60} L ${center + 45} ${
            correctedY - offset + 60
          } L ${center} ${correctedY - offset + 8}`}
          fill="black"
          fillTop={color}
...
const drawTick = (type: TickType, y: number, value: number): JSX.Element => {
  ...
  switch (type) {
    case TickType.ONE:
      return (
        <PathWithBlackBackground
          d={`M ${center - 0.5 * OneTickWidth} ${correctedY - offset} L ${
            center - 0.5 * OneTickWidth + OneTickWidth
          } ${correctedY - offset}`}
          fill="black"
          fillTop="white"
          strokeWidth={strokeWidth}
    ...
    case TickType.FOURTY_SIXTY_NINETY:
      return (
        <>
          <PathWithBlackBackground
            d={`M ${center - FourtyTickWidth + 35} ${correctedY - offset} L ${
              center + 35 + FourtyTickWidth
            } ${correctedY - offset}`}
            fill="black"
            fillTop="white"
            strokeWidth={strokeWidth}
...
const createPitchMarkings = (): JSX.Element[] => {
  return pitchArray.map((pitch) => {
    ...
    switch (pitchCorrected) {
      case -90:
        return drawTick(
          TickType.FOURTY_SIXTY_NINETY,
          pitchCorrected,
          Math.abs(pitchCorrected)
        );
      case -65:
...
export class AttitudeForeground extends DisplayComponent<AttitudeForegroundProps> {
  ...
  public onAfterRender(node: VNode): void {
    ...
    sub
      ...
      .whenChanged()
      .handle((bank) => {
        this.bankRef.instance?.setAttribute(
          "transform",
          `rotate(${bank.toString()}, 275, 255)`
        );
        this.bankRefDup.instance?.setAttribute(
          "transform",
          `rotate(${bank.toString()}, 275, 255)`
        );
      });
    sub
      ...
      .whenChanged()
      .handle((pitch) => {
        this.pitchRef.instance?.setAttribute(
          "transform",
          `translate(0,${this.getTranslation(-pitch).value * 8.6})`
        );
        this.pitchRefDup.instance?.setAttribute(
          "transform",
          `translate(0,${this.getTranslation(-pitch).value * 8.6})`
        );
        this.markingRef.instance?.setAttribute(
          "transform",
          `translate(0,${-pitch * 8.6})`
        );
