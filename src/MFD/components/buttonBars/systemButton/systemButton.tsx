...
export const SystemButton: React.FC<T_SystemButtonProps> = (
  props: T_SystemButtonProps
): JSX.Element => {
  const [systemPage, setSystemPage] = useObjLocalVar(
    "MFD_ROUTER_SYSTEM",
    "Number"
  );
      ...
      >
        <div className="system-button-center">
          <div className="system-button-children">
            {props.text}
            <div className="active-circle-outer">
              {isActive && <div className="active-circle-inner"></div>}
            </div>
          </div>
        </div>
