import { type T_FlightPlan } from "common/WaypointTypes/WaypointTypes";
...
import { useObjLocalVar, useSimVar } from "common/Hooks/simVars";
import { Icon as OgIcon, type Map as LeafletMap } from "leaflet";

type T_MapLayerProps = {
  sampleFlightPlan: T_FlightPlan;
  flightPlanPath: [number, number][];
  activeFlightPlanPath: [number, number][];
};
...
export const MapLayer: FC<T_MapLayerProps> = (props: T_MapLayerProps) => {
  ...
  const zoomRef = useRef<LeafletMap>(null);
  ...
  return (
    <div>
      ...
      <svg viewBox="0 0 600 455" className="compass-svg-mask">
        ...
        <path
          d="M 33, 315 A 265 265 0 1 1 567 315 L 600 315 L 600 0 L 0 0 L 0 315 L 33 315"
          fill="black"
        />
	...
	c-0.1-34.4-11.9-67.9-34.1-94.2C585.7,51,570.1,39,552.5,39z"
            ...
            <polygon
              fill="#666666"
              points="110.2,683.7 481,586.8 472.6,492.6 110.2,633.1 "
            />
            <polygon
              fill="#666666"
              points="394.3,1063.1 544.2,964.8 525.5,911.4 394.3,1019.1 "
            />
          </g>
        </g>
      </svg>
      <div className="map-wrapper" ref={mapRef}>
        ...
        >
          ...
          <Marker
            icon={waypointIcon}
            position={[
              props.sampleFlightPlan[0].lat,
              props.sampleFlightPlan[0].lon,
            ]}
          >
          ...
          <Marker
            icon={waypointIcon}
            position={[
              props.sampleFlightPlan[1].lat,
              props.sampleFlightPlan[1].lon,
            ]}
          >
            <Tooltip direction="top" offset={[0, -15]} permanent>
              <div
          ...
          <Marker
            icon={waypointIcon}
            position={[
              props.sampleFlightPlan[2].lat,
              props.sampleFlightPlan[2].lon,
            ]}
          >
          ...
          <Polyline
            pathOptions={inactivePathOptions}
            positions={props.flightPlanPath}
          />
          <Polyline
            pathOptions={activePathOptions}
            positions={props.activeFlightPlanPath}
          />
        </MapContainer>
      </div>
    </div>
