  ...
  const drawTick = (type: TickType, y: number, value: number): JSX.Element => {
    ...
    switch (type) {
      ...
      case TickType.FIVE:
        return (
          <PathWithBlackBackground
            d={`M ${center - 0.5 * FiveTickWidth} ${correctedY - offset} L ${center - 0.5 * FiveTickWidth + FiveTickWidth} ${
              correctedY - offset
            }`}
            fill="black"
            fillTop="white"
      ...
      case TickType.TEN:
        return (
          <>
            ...
            <PathWithBlackBackground
              d={`M ${center - 0.5 * TenTickWidth} ${correctedY - offset} L ${center - 0.5 * TenTickWidth + TenTickWidth} ${
                correctedY - offset
              }`}
              fill="black"
              fillTop="white"
